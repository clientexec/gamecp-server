...
class PluginGamecp extends ServerPlugin
{
            ...
            foreach ($packagevars as $key => $result) {
                if ($key == 'addon_tickrate' || $key == 'addon_fps' || $key == 'sv_slots') {
                    if (@$args['package']['addons'][$key] != "") {
                        ...
                        if ($result != "") {
                            $args['package']['addons'][$key] = $result;
                        }
                    }
                }
            }
    ...
    function set_all_args($args)
    {
        ...
        if ($iptype == "1") {
            ...
            $sv_location = $args['package']['addons']['sv_location'];
        }
    ...
    function ServiceExists($userPackageId)
    {
        ...
        $res= false;
        $res2= false;
        if (!isset($args['customer']['id']) || !isset($args['package']['id'])) {
            return false;
        }
        ...
        if (@$matches['name'][0] == $args['package']['username']) {
            $res= true;
        }
